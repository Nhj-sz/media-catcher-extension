...
- **B 站视频直链下载**：在 B 站播放页（含网页内悬浮面板与工具栏弹窗），可一键「解析可下载清晰度」并直接下载。优先获取**音视频合一的 MP4 直链**；仅分片流（DASH）时则分别下载视频与音频并提供合并命令。（原理：读取页面 `bvid`/`cid` → WBI 签名调用播放接口 → 取流直链，下载时自动注入 `Referer`。需登录高画质请保持 B 站网页已登录。）
